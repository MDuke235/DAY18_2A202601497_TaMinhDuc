...
def _get_semantic_model():
    """Lazy-load + cache sentence encoder (load 1 lần cho cả process)."""
...
def _pack(units: list[str], max_size: int, sep: str) -> list[str]:
    """Gộp units thành block ≤ max_size chars. Unit dài hơn max_size bị hard-split."""
    blocks: list[str] = []
    current = ""
    for u in units:
        while len(u) > max_size:                       # unit quá dài → cắt cứng
            if current:
                blocks.append(current.strip())
                current = ""
            blocks.append(u[:max_size].strip())
            u = u[max_size:]
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for parent_text in _pack(paragraphs, parent_size, "\n\n"):
        ...
        for child_text in _pack(sentences, child_size, " "):
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    chunks: list[Chunk] = []
    section = ""          # header hiện tại
    body = parts[0] if parts else ""

    def flush(section: str, body: str) -> None:
        full = f"{section}\n\n{body}".strip() if section else body.strip()
        if full:
            chunks.append(Chunk(
                text=full,
                metadata={**metadata, "section": section.lstrip("# ").strip(),
                          "header": section.strip(), "strategy": "structure",
                          "chunk_index": len(chunks)},
            ))

    for part in parts[1:]:
        if re.match(r"^#{1,3}\s+.+$", part.strip()):
            flush(section, body)      # đóng section trước
            section, body = part, ""
        else:
            body += part
